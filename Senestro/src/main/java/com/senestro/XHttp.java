...
import com.senestro.interfaces.XHttpCallback;
...
/**
 * Utility class for making HTTP(S) requests (GET, POST, HEAD). Provides an
 * all-trusting trust manager for HTTPS connections.
 *
 * <p>
...
public class XHttp {
     ...
     */
    public static void Get(@NonNull String url, @Nullable Map<String, String> headers, @Nullable Map<String, Object> parameters, @NonNull XHttpCallback callback) {
        try {
     ...
     * @param callback   The callback to handle the response or errors.
     */
    public static void Post(@NonNull String url, @Nullable Map<String, String> headers, @Nullable Map<String, Object> parameters, @NonNull XHttpCallback callback) {
        try {
            // Get trust manager that trust all certificates
            trustManager();
            // The boundary string
                ...
                try (OutputStream os = connection.getOutputStream(); PrintWriter writer = new PrintWriter(new OutputStreamWriter(os, StandardCharsets.UTF_8), true)) {
                    for (Map.Entry<String, Object> entry : parameters.entrySet()) {
                        ...
                        if (!Utils.isFile(value)) {
                            ...
                            writer.append("--").append(boundary).append(LINE);
                            writer.append("Content-Disposition: form-data; name=\"").append(field).append("\"; filename=\"").append(file.getName()).append(LINE);
     ...
     * @param callback The callback to handle the response or errors.
     */
    public static void Head(@NonNull String url, @Nullable Map<String, String> headers, @NonNull XHttpCallback callback) {
        try {
            // Get trust manager that trust all certificates
            trustManager();
            // Make an HTTPS request
    ...
    private static void trustManager() throws NoSuchAlgorithmException, GeneralSecurityException, IOException {
        ...
        sc.init(null, new TrustManager[]{Utils.trustManager(null)}, new SecureRandom());
    ...
    /**
     * Get's' the URL by removing any query string.
     *
     * @param url The URL .
