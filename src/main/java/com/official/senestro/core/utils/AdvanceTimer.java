...
import com.official.senestro.core.callbacks.interfaces.StaticTimerBackgroundCallback;
import com.official.senestro.core.callbacks.interfaces.StaticTimerCallback;
...
public class AdvanceTimer {
     ...
     */
    public static void schedule(StaticTimerCallback callback, long delay) {
     ...
     */
    public static void scheduleInBackground(StaticTimerBackgroundCallback callback, long delay) {
        new Timer().schedule(new TimerTask() {
            @Override
            public void run() {
     ...
     */
    public static void scheduleRepeat(StaticTimerCallback callback, int delay, int repeat) {
     ...
     */
    public static void scheduleRepeatInBackground(StaticTimerBackgroundCallback callback, int delay, int repeat) {
        Handler handler = new Handler(Looper.getMainLooper());
    ...
    private static class RepeatRunnable implements Runnable {
        ...
        private final StaticTimerCallback callback;
        private final int delay;
        private final int repeatEndTimes;
        private int endTimes = 0;

        public RepeatRunnable(Handler handler, StaticTimerCallback callback, int delay, int repeatEndTimes) {
    ...
    private static class RepeatRunnableInBackground implements Runnable {
        private final Handler handler;
        private final StaticTimerBackgroundCallback callback;
        private final int delay;
        private final int repeatEndTimes;
        private int endTimes = 0;

        public RepeatRunnableInBackground(Handler handler, StaticTimerBackgroundCallback callback, int delay, int repeatEndTimes) {
            this.handler = handler;
            this.callback = callback;
            this.delay = delay;
            this.repeatEndTimes = repeatEndTimes;
