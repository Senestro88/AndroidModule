...
public class OkHttpRequest {
    ...
    public void setRequestMethod(@NonNull Method method) {
        this.method = method;
    }
    ...
    public void execute(@NonNull Method method, @NonNull String url, @NonNull String tag, @Nullable OkHttpRequestCallback callback) {
        this.method = method;
        this.url = url;
        this.callback = callback;
        if (!methodValid()) {
            postOnCallback(listener -> listener.onErrorResponse(tag, "The request method is not valid"));
        } else {
            OkHttpClient client = getClient(url);
            if (client == null) {
                postOnCallback(listener -> listener.onErrorResponse(tag, "Failed to get OkHttpClient"));
            } else {
                ...
                try {
                    if (method == Method.GET) {
                        ...
                        requestBuilder.url(url).headers(headerBuilder.build()).method(convertMethod(), requestBody);
                    }
                    Request req = requestBuilder.build();
                    client.newCall(req).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NonNull Call call, @NonNull final IOException e) {
                            postOnCallback(listener -> listener.onErrorResponse(tag, e.getMessage()));
                        }
                        ...
                        public void onResponse(@NonNull Call call, @NonNull final Response response) {
                            try {
                                ...
                                for (String headerName : responseHeaders.names()) {
                                    headersMap.put(headerName, responseHeaders.get(headerName) != null ? responseHeaders.get(headerName) : "null");
                                }
                                postOnCallback(listener -> listener.onResponse(tag, responseBody, headersMap));
                            } catch (IOException e) {
                                postOnCallback(listener -> listener.onErrorResponse(tag, e.getMessage()));
                            }
                        }
    ...
    private void postOnCallback(CallbackExecutor executor) {
        if (callback != null) {
