...
public class AdvanceFilePicker {
    ...
    private final ArrayList<AdvanceFile> lists = new ArrayList<>();
    ...
    private AdvanceMaterialToast advanceMaterialToast;

    public AdvanceFilePicker(@NonNull Context context, @NonNull Activity activity) {
        this.context = context;
        this.activity = activity;
        storageUtils = new StorageUtils(this.context);
        advanceMaterialToast = new AdvanceMaterialToast(this.context);
    }
    ...
    private void createDialog(@NonNull CallbackListener callback) {
        ...
        view = activity.getLayoutInflater().inflate(R.layout.advance_file_picker, null);
    ...
    private void listPath(@NonNull String absolutePath) {
        ...
        if (AdvanceUtils.isDirectory(absPath)) {
            activity.runOnUiThread(() -> {
                ...
                List<AdvanceFile> files = android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q ? AdvanceUtils.listDir(context, new File(absolutePath), false) : AdvanceUtils.listDir(new File(absolutePath), false);
                lists.addAll(files);
    ...
    private ArrayList<AdvanceFile> listMPath() {
        ArrayList<AdvanceFile> array = new ArrayList<>(lists);
        lists.clear();
        if (mime != null) {
            for (AdvanceFile file : array) {
    ...
    private void normalToast(String message, int length) {
        advanceMaterialToast.normalToast(message, length).show();
    }

    private void successToast(String message, int length) {
        advanceMaterialToast.successToast(message, length).show();
    }

    private void errorToast(String message, int length) {
        advanceMaterialToast.errorToast(message, length).show();
    }

    private void infoToast(String message, int length) {
        advanceMaterialToast.infoToast(message, length).show();
    }
    ...
    public class ListViewAdapter extends BaseAdapter {
        ArrayList<AdvanceFile> data;

        public ListViewAdapter(ArrayList<AdvanceFile> array) {
            data = array;
        }
        ...
        @Override
        public AdvanceFile getItem(int index) {
        ...
        public View getView(int index, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = activity.getLayoutInflater().inflate(R.layout.advance_file_picker_list, null);
            ...
            AdvanceFile file = lists.get(index);
            ...
            activity.runOnUiThread(() -> {
                if (AdvanceUtils.isDirectory(realPath)) {
                    holder.thumbnail.setImageResource(R.mipmap.ic_file_picker_folder_black);
                } else {
                    if (AdvanceUtils.validImage(realPath)) {
                        holder.thumbnail.setImageResource(R.mipmap.ic_file_picker_image_black);
                    } else if (AdvanceUtils.validVideo(realPath)) {
                        holder.thumbnail.setImageResource(R.mipmap.ic_file_picker_movie_black);
                    } else {
                        holder.thumbnail.setImageResource(R.mipmap.ic_file_picker_extension_black);
                    }
                }
