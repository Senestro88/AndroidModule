...
public class AdvanceFile extends File {
    
    /**
     * Constructs a new {@code AdvanceFile} instance using a specified pathname.
     *
     * @param pathname the pathname of the file or directory (must not be
     * {@code null}).
     */
    public AdvanceFile(@NonNull String pathname) {
        super(pathname);
    }

    /**
     * Constructs a new {@code AdvanceFile} instance using a specified File object.
     *
     * @param file the file or directory object (must not be {@code null}).
     */
    public AdvanceFile(@NonNull File file) {
        super(getRealPath(file));
    }

    /**
     * Constructs a new {@code AdvanceFile} instance using a parent pathname and child
     * pathname string.
     *
     * @param parent the parent pathname string (nullable).
     * @param child the child pathname string (must not be {@code null}).
     */
    public AdvanceFile(@Nullable String parent, @NonNull String child) {
        super(parent, child);
    }

    /**
     * Constructs a new {@code AdvanceFile} instance using a parent {@link File}
     * object and child pathname string.
     *
     * @param parent the parent {@link File} object (nullable).
     * @param child the child pathname string (must not be {@code null}).
     */
    public AdvanceFile(@Nullable File parent, @NonNull String child) {
        super(parent, child);
    }
     ...
     */
    public AdvanceFile(@Nullable File parent, @NonNull File child) {
        super(parent, getRealPath(child));
    }

    /**
     * Constructs a new {@code AdvanceFile} instance using a URI.
     *
     * @param uri the URI representing the file (must not be {@code null}).
     */
    public AdvanceFile(@NonNull URI uri) {
        super(uri);
    }
    ...
    public String getExtension() {
        try {
            return isFile() ? AdvanceUtils.getExtension(getAbsolutePath()).toLowerCase() : "";
        } catch (Throwable e) {
            return "";
        }
    }
     ...
     */
    public long getSize() {
        return AdvanceUtils.getFileSize(getAbsolutePath());
    }
    ...
    /**
     * Lists {@link AdvanceFile} objects representing the files and directories within
     * this directory.
     *
     * <p>
     * This method checks if the current file is a directory and is readable. If
     * both conditions are met, it retrieves a list of {@link AdvanceFile} objects
     ...
     * @return a {@link List} of {@link AdvanceFile} objects representing the files
     * and directories within this directory if the conditions are met;
     * otherwise, an empty list.
     */
    public List<AdvanceFile> listXFiles(boolean recursively) {
        return isDirectory() && canRead() ? AdvanceUtils.listDir(this, recursively) : new ArrayList<>();
    }

    /**
     * A static comparator for comparing {@code AdvanceFile} objects.
     * <p>
     * Directories are prioritized over files, and both are sorted
     * alphabetically by name (case-insensitive).
     */
    public static class XFileComparator implements Comparator<AdvanceFile> {

        /**
         * Compares two {@code AdvanceFile} objects for order.
         *
         * @param a the first {@code AdvanceFile} object.
         * @param b the second {@code AdvanceFile} object.
         * @return a negative integer, zero, or a positive integer if the first
         * argument is less than, equal to, or greater than the second.
         */
        @Override
        public int compare(AdvanceFile a, AdvanceFile b) {
    ...
    // PRIVATE METHOD
    /**
     * Retrieves the absolute/real path of the file object
     *
