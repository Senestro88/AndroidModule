...
import com.official.senestro.core.callbacks.interfaces.AdvanceWifiEventsCallback;
...
public class AdvanceWifi {
    ...
    protected final @NonNull android.net.wifi.WifiManager wifi;
    protected final @NonNull ConnectivityManager connectivity;
    @SuppressLint("StaticFieldLeak")
    private static volatile AdvanceWifi instance;
    private android.net.wifi.WifiManager.LocalOnlyHotspotReservation hotspotStartedReservation;
    private boolean isReceiverRegistered = false;

    public AdvanceWifi(@NonNull Context context) {
        this.context = context;
        this.wifi = (android.net.wifi.WifiManager) context.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
    ...
    public static AdvanceWifi getInstance(@NonNull Context context) {
        if (instance == null) {
    ...
    /**
     * Get available network address. Default to loopback address (127.0.0.1) if no suitable IPv4 address is found
     *
     * @return String
     */
    ...
    /**
     * Open wifi settings for Android 10 and above
     */
    ...
    /**
     * Open tethering and portable hotspot settings
     */
    ...
    /**
     * Check to see if the app can modify system systems for Android 6.0 and above
     *
     * @return Boolean
     */
    ...
    /**
     * Open modify system systems for Android 6.0 and above if the permission is not granted
     *
     * @param force Force to open event if it's allowed
     */
    @RequiresApi(value = Build.VERSION_CODES.M)
    public void openWriteSettings(boolean force) {
        if (force || !canModifySystemSettings()) {
    ...
    /**
     * Get active  wifi connection information
     *
     * @return WifiInfo
     */
    ...
    /**
     * Get all the configured network
     *
     * @return Lists of WifiConfiguration
     */
    ...
    public int calculateSignalLevel(int rssi, int numLevels) {
        return android.net.wifi.WifiManager.calculateSignalLevel(rssi, numLevels);
    }
    ...
    public void startLocalOnlyHotspot(@NonNull AdvanceWifiLocalHotspotCallback callback) {
        closeWifi();
        this.wifi.startLocalOnlyHotspot(new android.net.wifi.WifiManager.LocalOnlyHotspotCallback() {
            @Override
    ...
    public static ArrayList<ScanResult> getScanResultsExcludeRepetition(@NonNull List<ScanResult> scanResults) {
        ...
        for (ScanResult scanResult : scanResults) {
            ...
            if (android.net.wifi.WifiManager.calculateSignalLevel(tempResult.level, 100) < android.net.wifi.WifiManager.calculateSignalLevel(scanResult.level, 100)) {
                hashMap.put(ssid, scanResult);
            }
        }
    ...
    public class WIFIReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            android.net.wifi.WifiManager manager = (android.net.wifi.WifiManager) context.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
                ...
                if (action != null) {
                    switch (action) {
                        ...
                        case android.net.wifi.WifiManager.RSSI_CHANGED_ACTION:
                            int newRssi = intent.getIntExtra(android.net.wifi.WifiManager.EXTRA_NEW_RSSI, 0);
    ...
    private void handleWiFiStateChanged(@NonNull Intent intent) {
        int state = intent.getIntExtra(android.net.wifi.WifiManager.EXTRA_WIFI_STATE, android.net.wifi.WifiManager.WIFI_STATE_UNKNOWN);
        switch (state) {
