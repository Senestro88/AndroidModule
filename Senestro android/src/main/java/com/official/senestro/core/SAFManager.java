...
public class SAFManager {
    private final String tag = SAFManager.class.getName();
    ...
    public ArrayList<HashMap<String, Object>> listTree(@NonNull Uri treeUri) {
        ...
        if (validUri(treeUri) && isDir(treeUri)) {
            ...
            Uri childrenDocumentsUri = DocumentsContract.buildChildDocumentsUriUsingTree(treeUri, treeDocumentId); // Build the Uri for the children of the picked directory
            ...
            Cursor cursor = contentResolver.query(childrenDocumentsUri, columns, null, null, null);
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    try {
                        ...
                        map.put("size", size);
                        map.put("mime", mime);
                        map.put("absolutePath", getPathFromUri(uri));
                        lists.add(map);
                    } catch (Throwable e) {
                        Log.e(tag, e.getMessage(), e);
                    }
                }
                XUtils.closeQuietly(cursor);
            }
        }
        return lists;
    }

    public ArrayList<HashMap<String, Object>> listChildTree(@NonNull Uri treeUri, @NonNull Uri childUri) {
        ArrayList<HashMap<String, Object>> lists = new ArrayList<>();
        if (validUri(treeUri) && validUri(childUri) && isDir(treeUri) && isDir(childUri)) {
            ...
            if (childCursor != null) {
                ...
                XUtils.closeQuietly(childCursor);
            }
        }
        return lists;
    }

    public boolean validUri(@NonNull Uri uri) {
        try {
            ContentResolver contentResolver = context.getContentResolver();
    ...
    public boolean isDir(@NonNull Uri uri) {
        try {
    ...
    public boolean isFile(@NonNull Uri uri) {
        try {
            String mime = getMimeFromUri(uri);
            return mime != null && !mime.equalsIgnoreCase(DocumentsContract.Document.MIME_TYPE_DIR);
        } catch (Throwable e) {
    ...
    public String getMimeFromUri(@NonNull Uri uri) {
        String mime = null;
        try {
            ContentResolver contentResolver = context.getContentResolver();
            Uri uriDecoded = Uri.parse(URLDecoder.decode(String.valueOf(uri), "UTF-8"));
    ...
    public String getPathFromUri(@NonNull Uri uri) {
        return XUtils.convertUriToFilePath(context, uri);
    }

    public void deleteChildUri(@NonNull Uri treeUri, @NonNull Uri childUri) {
        try {
            ContentResolver contentResolver = context.getContentResolver();
    ...
    // ================================================= //
    private void deleteUriDirRecursively(@NonNull ContentResolver contentResolver, @NonNull Uri treeUri, @NonNull Uri directoryUri) {
