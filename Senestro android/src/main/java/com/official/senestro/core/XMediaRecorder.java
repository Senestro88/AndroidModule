...
import com.official.senestro.core.callbacks.interfaces.XMediaRecorderCallback;
...
public class XMediaRecorder {
    private final String tag = XMediaRecorder.class.getName();
    ...
    private XMediaRecorderCallback callback;
    ...
    public XMediaRecorder(@NonNull Context context, @NonNull Activity activity) {
    ...
    public void setCallback(@Nullable XMediaRecorderCallback callback) {
            ...
            if (!startedRecording) {
                ...
                if (XUtils.notNull(filename)) {
                    try {
                        ...
                        postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while starting the voice record."));
                    }
                } else {
            ...
            if (isRecording()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    try {
                        ...
                        postToCallback(XMediaRecorderCallback::onPaused);
                    } catch (Throwable e) {
                        Log.e(tag, e.getMessage(), e);
                        String message = e.getMessage();
                        postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while pausing the recorder."));
                    }
                } else {
            ...
            if (isPaused()) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    try {
                        ...
                        onPauseTime = 0;
                        postToCallback(XMediaRecorderCallback::onResumed);
                    } catch (Throwable e) {
                        Log.e(tag, e.getMessage(), e);
                        String message = e.getMessage();
                        postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while resuming the voice record."));
                    }
                } else {
        ...
        XExecutorService.runInBackground(() -> {
            if (startedRecording) {
                try {
                    ...
                    Log.e(tag, e.getMessage(), e);
                    String message = e.getMessage();
                    postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while stopping the voice record."));
                }
            } else {
        ...
        endlessTaskHandler.start(() -> {
            if (startedRecording && isRecording()) {
                try {
                    ...
                    Log.e(tag, e.getMessage(), e);
                    String message = e.getMessage();
                    postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred on the record runnable."));
                }
            }
        });
    }
    ...
    private String createFilename(@NonNull OutputFormat format) {
        if (XUtils.isDirectory(dir)) {
            ...
            if (XUtils.notNull(extension)) {
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd_HHmmssSSS", Locale.US);
                return dir + File.separator + dateFormat.format(new Date()).concat(extension);
            }
        }
        return null;
    }
    ...
    private class BSVRMethods {
        ...
        private void source(@NonNull MediaRecorder mediaRecorder, @NonNull Source source) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the source."));
            }
        }
        ...
        private void format(@NonNull MediaRecorder mediaRecorder, @NonNull OutputFormat format) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the format."));
            }
        }
        ...
        private void encoder(@NonNull MediaRecorder mediaRecorder, @NonNull OutputFormat format, @NonNull Bitrate bitrate) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the encoder."));
            }
        }
        ...
        private void bitrate(@NonNull MediaRecorder mediaRecorder, @NonNull Bitrate bitrate) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the bitrate."));
            }
        }
        ...
        private void output(@NonNull MediaRecorder mediaRecorder, @NonNull String filename) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the output filename."));
            }
        }
        ...
        private void maxDuration(@NonNull MediaRecorder mediaRecorder, int durationMillis) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the output max duration."));
            }
        }
        ...
        private void maxBytes(@NonNull MediaRecorder mediaRecorder, int sizeBytes) {
            try {
                ...
                Log.e(tag, e.getMessage(), e);
                String message = e.getMessage();
                postToCallback(listener -> listener.onError(!XUtils.isNull(message) ? message : "An error has occurred while setting the output max file size."));
            }
        }
    }
    ...
    private interface CallbackExecutor {
        void execute(XMediaRecorderCallback listener);
    }
