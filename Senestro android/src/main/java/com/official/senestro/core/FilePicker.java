...
public class FilePicker {
    ...
    private final ArrayList<XFile> lists = new ArrayList<>();
    ...
    public void startPicker(@NonNull CallbackListener callback) {
        XTimer.schedule(() -> activity.runOnUiThread(() -> createDialog(callback)), 100);
    }
    ...
    private void setButtonListeners(CallbackListener callback) {
        ...
        backButton.setOnClickListener(v -> {
            if (!currentPath.equals(rootPath)) {
                listPath(XUtils.getParentPath(currentPath));
            }
        });
    ...
    private void listPath(@NonNull String absolutePath) {
        ...
        if (XUtils.isDirectory(absPath)) {
            activity.runOnUiThread(() -> {
                ...
                List<XFile> files = android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q ? XUtils.listDir(context, new File(absolutePath), false) : XUtils.listDir(new File(absolutePath), false);
                lists.addAll(files);
    ...
    private ArrayList<XFile> listMPath() {
        ArrayList<XFile> array = new ArrayList<>(lists);
        lists.clear();
        if (mime != null) {
            for (XFile file : array) {
    ...
    private void clickEvent(View view, int index, String realPath) {
        view.setOnClickListener(v -> {
            if (XUtils.isDirectory(realPath)) {
                listPath(realPath);
            } else {
    ...
    public class ListViewAdapter extends BaseAdapter {
        ArrayList<XFile> data;

        public ListViewAdapter(ArrayList<XFile> array) {
            data = array;
        }
        ...
        @Override
        public XFile getItem(int index) {
        ...
        public View getView(int index, View convertView, ViewGroup parent) {
            ...
            XFile file = lists.get(index);
            String realPath = file.getAbsolutePath();
            holder.name.setText(XUtils.getPathLastSegment(realPath));
            holder.size.setText(XUtils.readableSize(realPath));
            final View currentView = convertView;
            activity.runOnUiThread(() -> {
                if (XUtils.isDirectory(realPath)) {
                    holder.thumbnail.setImageResource(R.mipmap.files_picker_ic_folder_black);
                } else {
                    if (XUtils.validImage(realPath)) {
                        holder.thumbnail.setImageResource(R.mipmap.files_picker_ic_image_black);
                    } else if (XUtils.validVideo(realPath)) {
