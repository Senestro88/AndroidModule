...
import com.official.senestro.core.XFile;
...
public class XUtils {
    private static final String TAG = XUtils.class.getName();
    ...
    public static List<XFile> listDir(@NonNull File dirFile, boolean recursively) {
        ...
        if (dirFile.isDirectory()) {
            ...
            if (listedFiles != null) {
                List<XFile> infoUtils = new ArrayList<>();
                for (File list : listedFiles) {
                    infoUtils.add(new XFile(list.getAbsolutePath()));
                }
                Collections.sort(infoUtils, new XFile.XFileComparator());
                for (XFile list : infoUtils) {
    ...
    public static List<XFile> listDir(@NonNull Context context, @NonNull File dirFile, boolean recursively) {
        List<XFile> lists = new ArrayList<>();
        if (dirFile.isDirectory()) {
    ...
    @RequiresApi(api = Build.VERSION_CODES.Q)
    public static List<XFile> listDir(@NonNull Context context, @NonNull String dirPath, boolean recursively) {
    ...
    public static String randomBasename(@NonNull String extension) {
        return XUtils.generateRandomText(20) + Calendar.getInstance().getTimeInMillis() + "." + extension;
    }
    ...
    public static void deleteEmptyDirectories(@NonNull String path) {
        ArrayList<String> emptyDirectories = XUtils.getEmptyDirectories(path);
    ...
    public static ArrayList<HashMap<String, Object>> getPackagesInfoData(@NonNull Context context, @NonNull List<PackageInfo> packagesInfo) {
        ...
        try {
            ...
            for (PackageInfo packageInfo : packagesInfo) {
                ...
                data.put("appSize", new XFile(sourceDir).length());
    ...
    public static void openBrowser(@NonNull Context context, @NonNull String url) throws Exception {
        String defaultBrowserPackageName = XUtils.getDefaultBrowserPackageName(context);
        ...
        if (intent != null) {
            try {
                if (intent.resolveActivity(context.getPackageManager()) != null) {
                    ...
                    XUtils.createDirectory(directory);
    ...
    public static String getBasename(@NonNull String absolutePath) {
        return XUtils.getPathLastSegment(absolutePath);
    }
    ...
    public static X509TrustManager unsafeTrustManager(@Nullable XFile certFile) throws GeneralSecurityException, IOException {
    ...
    public static OkHttpClient getUnsafeOkHttpClient(@Nullable XFile certFile, int socketTimeout, int readTimeout) {
        try {
